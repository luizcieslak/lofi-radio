...
> **Resolved:** _Inconsistent track loudness → volume jumps between songs._ Every track is now loudness-normalized to **-14 LUFS / -1 dBTP / 11 LU** (EBU R128, streaming standard) via two-pass `loudnorm`. This is folded into the SAME re-encode as the format normalization in [src/audioNormalizer.ts](src/audioNormalizer.ts) (one generational loss), wired into `POST /admin/upload` + `/admin/upload/batch`. A file is left bit-for-bit untouched only when it is already at the canonical rate AND within `LOUDNESS_TOLERANCE_LU` (1.0 LU) of target; otherwise it's re-encoded. The existing production library was migrated by the offline `normalize-volume.ts` + `upload-volume-normalized.ts` tools in `~/clawd/lofi-radio-tools` (all 113 tracks; library loudness spread went from ~14 LU to ~1 LU, live stream now measures ~-14 LUFS).
...
- ✅ ~~Volume Normalization~~ - **Implemented.** All tracks loudness-normalized to -14 LUFS / -1 dBTP / 11 LU (EBU R128 two-pass `loudnorm`), folded into the upload re-encode in [src/audioNormalizer.ts](src/audioNormalizer.ts); existing library migrated via `normalize-volume.ts` + `upload-volume-normalized.ts` in `~/clawd/lofi-radio-tools`. See the Resolved note above.
